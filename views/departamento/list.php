...
<div id="box-list" class="well bs-component col-lg-10 col-lg-offset-1 col-md-8 col-md-offset-2 col-sm-8 col-sm-offset-2 col-xs-12">
    ...
    <a href="?op=new_departamento" id="btn-add" class="btn btn-success btn-fab">
    ...
    <table id="grid-data" class="table table-condensed table-hover table-striped" style="width:100%">
        <thead>
            <tr>
                <th data-column-id="pk_departamento_id" data-type="numeric">ID</th>
...
<script>
    ...
    $(document).ready(function(){
        $.post("request/departamento.php", {action: "read"}, function (data) {
            ...
            if (result.estado = 1 && result.resultado != null) {
                $.each(result.resultado, function (key, obj) {
                    $("#" + tableId + ' tbody').append('<tr>' +
                        '<td>' + obj.pk_departamento_id + '</td>' +
                        '<td>' + obj.pais + '</td>' +
                        '<td>' + obj.codigo + '</td>' +
                        '<td>' + obj.nombre + '</td>' +
                        '<td>' + obj.estado + '</td>' +
                        '<td>' +
                        '<button type="button" class="btn btn-xs btn-default" command="edit" data-row-id="' + obj.pk_departamento_id + '"><span class="glyphicon glyphicon-pencil"></span></button>' +
                        '<button type="button" class="btn btn-xs btn-default" command="delete" data-row-id="' + obj.pk_departamento_id + '"><span class="glyphicon glyphicon-trash"></span></button>' +
                        '</td>' +
                        '</tr>');
                });
            }
            ...
            $('#' + tableId + ' tbody').on('click', 'button', function () {
                ...
                DataTableActions(action, id, "edit_departamento", "request/departamento.php");
            });
        });
    });
</script>